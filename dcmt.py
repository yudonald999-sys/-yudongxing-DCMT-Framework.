...
from .boundary_detector import AdaptiveBoundaryDetector
...
class DCMTModel(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            return_alignment: Whether to return alignment scores
            
        Returns:
            DCMTOutput containing logits, chunks, alignment, and optional loss
        """
        # Encode modalities
        ...
        # h^l = TransformerBlock(h^{l-1} + TopDown(h^{l+1}))  [Equation 2]
        ...
        # L_align = -log(exp(S(V_i, t_i)/τ) / Σ_j exp(S(V_i, t_j)/τ))  [Equation 3]
        ...
        return DCMTOutput(
            logits=logits,
            chunks=torch.stack([visual_boundaries, text_boundaries], dim=1) if return_chunks else None,
            alignment=alignment_output['similarity'] if return_alignment else None,
            loss=loss
        )
